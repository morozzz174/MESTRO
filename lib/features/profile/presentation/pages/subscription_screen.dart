...
class _SubscriptionScreenState extends State<SubscriptionScreen> {
  ...
  Widget _buildFeaturesList() {
    const features = [
      ('🤖', 'AI-ассистент замерщика', 'Автоматический анализ данных замера и поиск ошибок'),
      ('⚠️', 'Проверка ошибок', 'Поиск аномалий, пропущенных полей и несоответствий СНиП'),
      ('💡', 'Умные рекомендации', 'Советы по материалам, технологиям и оптимизации'),
      ('💰', 'Анализ стоимости', 'Проверка корректности расчётов и сравнение с рынком'),
      ('🎙️', 'Голосовой ввод для 15 специализаций', 'Диктуйте замеры — AI заполнит все поля автоматически'),
      ('✅', 'AI-валидация КП', 'Проверка коммерческого предложения перед отправкой клиенту'),
      ('📝', 'Авто-заметки замерщика', 'AI генерирует заметки из голосового описания объекта'),
      ('📊', 'Авто-отчёты', 'Генерация резюме по замеру в PDF'),
    ];
